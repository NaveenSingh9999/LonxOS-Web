...
function getCachedRepo(url) {
    ...
    if (typeof cached === 'string') {
        try {
            const repoIndex = JSON.parse(cached);
...
function setCachedRepo(url, packages) {
    const repoIndex = {
        packages,
...
async function fetchAndIndexRepo(url) {
    try {
...
async function showRepoInfo(url) {
    const sources = getRepos();
