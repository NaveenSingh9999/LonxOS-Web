...
async function askQuestion(prompt, validation, errorMessage) {
    let answer = '';
    while (true) {
...
export async function runOnboardingWizard() {
    ...
    const username = await askQuestion("Enter your desired username (no spaces or symbols): ", (input) => /^[a-zA-Z0-9]+$/.test(input), "Invalid username. Please use only letters and numbers.");
    let hostname = await shell.readLine("Enter your hostname (default: lonx-machine): ");
    if (!hostname.trim()) {
        hostname = 'lonx-machine';
    }
    const theme = await askQuestion(`Choose a theme [${THEME_OPTIONS.join(' / ')}]: `, (input) => THEME_OPTIONS.includes(input), "Invalid theme. Please choose from the list.");
    const shellStyle = await askQuestion(`Choose a shell style [${SHELL_STYLE_OPTIONS.join(' / ')}]: `, (input) => SHELL_STYLE_OPTIONS.includes(input), "Invalid shell style. Please choose from the list.");
    ...
    shell.print("---------------------------\n");
    const confirmation = await askQuestion("Is this correct? (yes/no): ", (input) => ['yes', 'no'].includes(input.toLowerCase()), "Please enter 'yes' or 'no'.");
